...
public class RoomController : MonoBehaviourPunCallbacks
{
    ...
    void ListPlayers() 
    {
        ...
        foreach (Player p in PhotonNetwork.PlayerList) //loop through each player and create a player listing
        {
            GameObject tempListing = Instantiate(playerListingPrefab, playersContainer);
            TextMeshProUGUI tempText = tempListing.transform.GetChild(0).GetComponent<TextMeshProUGUI>();
            tempText.text = p.NickName;
            Debug.Log(p.ActorNumber);
            
        }
        Player player = PhotonNetwork.LocalPlayer;
        Persistant.team = player.ActorNumber;
    }
